#! /bin/bash
BIN_HOME="$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")"
cd "$BIN_HOME"
#
...
# Default-Stop: 0 1 6
# Short-Description: a http & socks proxy
# Description: a http & socks proxy
### END INIT INFO

# Application
APP_NAME="jproxy"
APP_LONG_NAME="JProxy"
...
removedaemon() {
    ...
    if [ "$DIST_OS" = "solaris" ] ; then
        ...
        if [ -f /etc/rc.bak ] ; then
            ...
            eval echo `gettext ' Removing $APP_LONG_NAME daemon...'`
            cp /etc/rc /etc/rc.bak
            sed  "s/_BPX_JOBNAME=\'APP_NAME\'.*" /etc/rc.bak > /etc/rc
            rm /etc/rc.bak
        else
            eval echo `gettext ' The $APP_LONG_NAME daemon is not currently installed.'`
            exit 1
